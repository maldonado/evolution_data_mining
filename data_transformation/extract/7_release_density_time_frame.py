...
try:
    ...
    for project_data in projects:
        ...
        for x in xrange(0,55):
            if x != 55 :
                # previous way to calculate = cursor.execute("select count(version) as release_density from metrics_data where project_name = '"+project_name+"' and release_date >= '"+str(date_array[x])+"' and release_date <= '"+str(date_array[x+1])+"'")
                cursor.execute("select count(distinct(version)) as release_density from commits where project_name = '"+project_name+"' and author_date >= '"+str(date_array[x])+"' and author_date <= '"+str(date_array[x+1])+"'")
                
                release_density = cursor.fetchall()
                value = release_density[0][0]
                print "insert into release_density(project_name, date_group, number_of_releases) values ('"+project_name+"', '"+date_array[x]+"', "+str(value)+")"
