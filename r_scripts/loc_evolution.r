...
mongoose <- dbSendQuery(con, "select project_name, release_date, lines_of_code as count from metrics_data where project_name in ('node-mongodb-native', 'mongo-java-driver', 'node-mysql', 'mysql-connector-j') order by 2")
