# complexity per release
library(RPostgreSQL)
drv <- dbDriver("PostgreSQL")
con <- dbConnect(drv, host='localhost', port='5432', dbname='jsevolution', user='evermal', password='')
mongoose <- dbSendQuery(con, "select project_name, release_date, complexity as count from metrics_data where project_name in ('node-mongodb-native', 'mongo-java-driver', 'node-mysql', 'mysql-connector-j') order by 2")
...
ggplot(data = data1, aes(x = Date, y = data1$count, colour = data1$project_name)) +       
  ...
  ylab("Complexity") + labs(colour='Projects')
