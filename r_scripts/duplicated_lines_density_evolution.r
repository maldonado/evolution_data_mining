...
mongoose <- dbSendQuery(con, "select project_name, release_date, duplicated_lines_density as count from metrics_data where project_name in ('node-mongodb-native', 'mongo-java-driver', 'node-mysql', 'mysql-connector-j') order by 2")
...
ggplot(data = data1, aes(x = Date, y = data1$count, colour = data1$project_name)) +       
  ...
  ylab("Duplication density") + labs(colour='Projects')
